...
class NodeGIN(nn.Module):
    ...
    def forward(self, g, h):
        ...
        h : tensor of dtype float32 and shape (B1, D1)
            Node features. B1 for number of nodes and D1 for
            the node feature size.

        Returns
        -------
        res : hidden_rep, all layers outputs with size of num_layers + 1
        """
        if isinstance(g, list):
            assert len(g)==self.num_layers
        else:
            g = [g] * self.num_layers
        ...
        for i in range(self.num_layers):
            h = self.ginlayers[i](g[i], h)
            h = self.batch_norms[i](h)
            h = F.relu(h)
            hidden_rep.append(h)

        # return all layers outputs with size of num_layers + 1
        return hidden_rep


    # def minibatch_forward(self, blocks, h, efeat):
    #     # list of hidden representation at each layer (including input)
    #     hidden_rep = [h]
...
class RGCN(nn.Module):
    ...
    # use_selayer=False,
    def __init__(self, in_feats, hid_feats, rel_names, num_layers,):
        super().__init__()
    ...
    def forward(self, g, h):
        ...
        for i in range(self.num_layers):
            h = self.ginlayers[i](g[i], h)
            h = {k: self.batch_norms[i](F.relu(v)) for k, v in h.items()}
            hidden_rep.append(h)
