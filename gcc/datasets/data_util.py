...
def batcher_pl():
    def batcher_dev(batch):
        pos_graph, neg_graph, subgs = zip(*batch)
        pos_graph, neg_graph, subgs = dgl.batch(pos_graph), dgl.batch(neg_graph), dgl.batch(subgs)
        return pos_graph, neg_graph, subgs
    return batcher_dev

def batcher_neg():
    def batcher_dev(batch):
        graph_q, graph_k, neg_q = zip(*batch)
        graph_q, graph_k, neg_q = dgl.batch(graph_q), dgl.batch(graph_k), dgl.batch(neg_q)
        return graph_q, graph_k, neg_q
...
def _rwr_trace_to_dgl_hetegraph(
    g, seed, trace, eids, positional_embedding_size #, entire_graph=False
):
    ...
    seed_ntype = g.ntypes[eids[0]]
    subv = {}
    for e,t in zip(eids, trace):
        if t not in [seed, -1]:
            ntype = g.ntypes[e]
            subv[ntype] = subv.get(ntype, [])+[t]
    for k,v in subv.items():
        subv[k] = torch.unique(torch.tensor(v)).tolist()
    subv[g.ntypes[eids[0]]] = [seed] + subv.get(g.ntypes[eids[0]],[])
    # print("subv:",subv)
    ...
    # subg.ndata["seed"] = {seed_ntype : torch.zeros(subg.number_of_nodes(ntype=seed_ntype), dtype=torch.long, device=subg.device)}
    subg.ndata["seed"][seed_ntype][0] = 1
    # subg.ndata["oridx"][seed_ntype]=seed * torch.ones(subg.number_of_nodes(ntype=seed_ntype) , dtype=torch.long, device=subg.device)
    return subg #, torch.tensor(subv, dtype=torch.long)
...
def _rwr_trace_to_dgl_graph(
    ...
    subg.ndata["seed"] = torch.zeros(subg.number_of_nodes(), dtype=torch.long, device=subg.device)
    if entire_graph:
        subg.ndata["seed"][seed] = 1
    else:
        subg.ndata["seed"][0] = 1
    subg.ndata["oridx"]=seed * torch.ones(subg.number_of_nodes() , dtype=torch.long, device=subg.device)
    return subg #, torch.tensor(subv, dtype=torch.long)
...
def _add_undirected_graph_positional_embedding(g, hidden_size, retry=10):
    ...
    data = adj.val
    shape = adj.shape
    adj = sparse.csr_matrix((data.cpu(), (row.cpu(), col.cpu())), shape=shape)
    ...
    g.ndata["pos_undirected"] = x.float().to(g.device)
    return g


def get_weibo_dgl_graph(
            data_path = "/data/zhangz/chenyilong/final_pj/another_task/tmp_data/weibo/data_preprocess/",
            code_path = "/data/zhangz/chenyilong/final_pj/another_task/tmp_data/weibo/code/"
    ...
    # path
    # adj_matrix
    df_emoji_train = pd.read_pickle(data_path + "weibo_emoji.pkl")
    post_feature = torch.load(code_path + 'BERT_code.pth', map_location=device)
    emoji_feature = torch.load(code_path + 'emoji_gcn.pth', map_location=device)
    adj_matrix = torch.tensor(df_emoji_train.values).to(device)
    edge_index, _ = torch_geometric.utils.dense_to_sparse(adj_matrix)  # 邻接矩阵转换边列表
    post_num, emoji_num = adj_matrix.shape[0], adj_matrix.shape[1]
    edge_index[1] = edge_index[1]+post_num  # index补齐
    edge_index = torch_geometric.utils.to_undirected(edge_index)  # 对称操作

    # node feature
    embedding_len = post_feature.shape[1]
    node_feature = torch.concat([post_feature, emoji_feature])
    node_label_post = torch.zeros(post_num).to(device)
    node_label_emoji = (torch.zeros(emoji_num)+1).to(device)
    node_label = torch.cat([node_label_post, node_label_emoji]).int()
    g = dgl.graph((edge_index[0], edge_index[1]))
    g.ndata["emb"] = node_feature
    g.ndata["label"] = node_label.to(torch.long)
    g.ndata["nindex"] = torch.arange(0,post_num+emoji_num, dtype=torch.long)
    return g


if __name__=='__main__':
    g=get_weibo_dgl_graph()
    print(g)